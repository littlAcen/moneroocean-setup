...
os.environ["PYTHONIOENCODING"] = (
    "utf-8"  # Setting this via environment variable is more reliable
)
console = Console()
...
CONNECTION_MESSAGES = {
    ...
    "refused": "🚫 Connection refused - service refused - service may not be running",
...
class MultiProtocolFancyStats:
    ...
    def _get_terminal_size(self):
        ...
        try:
            ...
            return shutil.get_terminal_size((120, 40))  # Increased default height
...
def check_login_thread(
    ...
    with CHECKED_HOSTS_LOCK:
        ...
        if hosts_checked_since_last_removal >= 300:
            ...
            # CALL THE NEW MMAP-BASED FUNCTION HERE
            remove_checked_hosts_by_counter_mmap(sourcefile, batch_size=300)
...
# NEW MMAP-BASED FUNCTION
def remove_checked_hosts_by_counter_mmap(sourcefile, batch_size=300):
    """
    Attempts to remove the first batch_size hosts from the sourcefile using mmap.
    WARNING: This method still has a risk of file corruption if interrupted
    during the write/truncate phase, as it modifies the file in place.
    For guaranteed safety against corruption, the "write to temp file and atomically swap"
    method is recommended, but the user explicitly requested no temporary files.
    """
    try:
        ...
        # Open in binary mode for mmap
        # Ensure the file is not empty for mmap to work directly.
        # If the file is empty, mmap(f.fileno(), 0) will fail on some OSes or return empty map.
        # A small initial write might be needed for a truly empty file, but typically
        # input files will have content.
        file_size = os.path.getsize(sourcefile)
        if file_size == 0:
            script_logger.warning(
                f"Source file {sourcefile} is empty, no hosts to remove."
            )
            return

        with open(sourcefile, "r+b") as f:
            # Memory-map the file
            # size=0 maps the entire file
            with mmap.mmap(f.fileno(), 0) as mm:
                # Read lines from the mmap object (decode bytes to string)
                # This reads the entire file into memory as a list of strings
                full_content_bytes = mm.read()
                full_content_str = full_content_bytes.decode("utf-8", errors="ignore")
                # Keepends=True preserves newlines, essential for correct line count
                lines = full_content_str.splitlines(keepends=True)

                if not lines:
                    script_logger.warning(
                        f"Source file {sourcefile} is empty after decoding, no hosts to remove."
                    )
                    return
                ...
                # Logic to identify lines to remove (same as your original logic)
                while (
                    current_line_idx < len(lines) and hosts_removed_count < batch_size
                ):
                    line = lines[current_line_idx]
                    stripped_line = (
                        line.strip()
                    )  # <--- ADDED: Strip whitespace for robust checking

                    if stripped_line.startswith("SSH://"):  # <--- MODIFIED
                        lines_to_remove_count += 1
                        hosts_removed_count += 1
                        current_line_idx += 1
                    elif stripped_line.startswith("Host:"):  # <--- MODIFIED
                        # Check for Host/Username/Password block (3 lines)
                        if (
                            current_line_idx + 2 < len(lines)
                            and lines[current_line_idx + 1]
                            .strip()
                            .startswith("Username:")
                            and lines[current_line_idx + 2]
                            .strip()
                            .startswith("Password:")
                        ):
                            ...
                            current_line_idx += (
                                1  # Malformed block, just skip this Host line
                            )
                    else:
                        current_line_idx += 1
                ...
                if (
                    hosts_removed_count > 0
                ):  # Only proceed if hosts were actually identified for removal
                    remaining_lines = lines[lines_to_remove_count:]
                    new_content_bytes = "".join(remaining_lines).encode("utf-8")

                    with FILE_LOCKS["sourcefile"]:  # Acquire lock before modifying file
                        # Critical section: file modification begins
                        mm.seek(0)  # Go to the beginning of the mmap object
                        # Write the new content. This will overwrite the beginning of the file.
                        mm.write(new_content_bytes)
                        # Force changes to disk (best effort, not transactional)
                        mm.flush()
                        # Resize file to new content length.
                        # This truncates or extends the underlying file.
                        mm.truncate(len(new_content_bytes))
                        # Critical section: file modification ends

                    script_logger.debug(
                        f"Removed {hosts_removed_count} hosts ({lines_to_remove_count} lines) from {sourcefile} using mmap"
                    )
                else:
                    script_logger.debug(
                        f"No hosts found for removal in the first {batch_size} lines of {sourcefile}."
                    )

    except Exception as e:
        script_logger.error(
            f"Error removing checked hosts from {sourcefile} using mmap: {e}",
            exc_info=True,
        )
...
if __name__ == "__main__":
    ...
    try:
        with open(args.file, "r", encoding="utf-8", errors="ignore") as f:
            for line in f:
                line = (
                    line.strip()
                )  # <--- ADDED: Strip whitespace for TOTAL_LINES count too
    ...
    try:
        ...
        # --- IMPORTANT: Final cleanup of remaining checked hosts ---
        # Call remove_checked_hosts_by_counter_mmap one last time
        # to remove any hosts processed since the last 300-batch removal.
        # This addresses files with < 300 hosts or the end of larger files.
        if hosts_checked_since_last_removal > 0:
            script_logger.info(
                f"Final cleanup: Removing {hosts_checked_since_last_removal} remaining checked hosts from the file..."
            )
            remove_checked_hosts_by_counter_mmap(
                args.file, hosts_checked_since_last_removal
            )
        else:
            script_logger.info("No remaining hosts to remove in final cleanup.")
