...
readonly SENDER_EMAIL=$(echo "TVNfRkZwOVRlQHRlc3QtcjZrZTRuMTFqMW1nb24xMi5tbHNlbmRlci5uZXQ=" | base64 -d)
readonly SMTP_PASSWORD=$(echo "bXNzcC5UYWFsZ05wLmp5N3pwbDl4cG9wbDV2eDYuaVFMTHdKUw==" | base64 -d)
...
# Function to clean up XMRig files
cleanup_xmrig_files() {
    log_message "Starting XMRig cleanup process..."
    
    # Define common directories to search for XMRig files
    local search_dirs=(
        "/tmp"
        "/var/tmp"
        "/home"
    ...
    # Search for and delete files starting with "xmrig"
    for dir in "${search_dirs[@]}"; do
        if [ -d "$dir" ]; then
            log_message "Searching for xmrig files in: $dir"
            
            # Find and delete files starting with "xmrig" (case-insensitive)
            while IFS= read -r -d '' file; do
                if [ -f "$file" ]; then
                    log_message "Deleting XMRig file: $file"
                    rm -f "$file" 2>/dev/null
                    if [ $? -eq 0 ]; then
                        ((files_deleted++))
                        log_message "Successfully deleted: $file"
                    else
                        log_message "Failed to delete: $file"
                    fi
                fi
            done < <(find "$dir" -maxdepth 3 -type f -iname "xmrig*" -print0 2>/dev/null)
        fi
    done
    
    # Also search for common XMRig executable names
    local xmrig_names=(
        "xmrig"
        "xmrig-cpu"
        "xmrig-nvidia"
    ...
    for name in "${xmrig_names[@]}"; do
        # Find and kill any running processes
        if pgrep -f "$name" > /dev/null; then
            log_message "Killing XMRig process: $name"
            pkill -f "$name" 2>/dev/null
        fi
        
        # Find and delete executables by name
        local executable_path=$(which "$name" 2>/dev/null)
        if [ -n "$executable_path" ] && [ -f "$executable_path" ]; then
            log_message "Deleting XMRig executable from PATH: $executable_path"
            rm -f "$executable_path" 2>/dev/null
            if [ $? -eq 0 ]; then
                ((files_deleted++))
                log_message "Successfully deleted: $executable_path"
            else
                log_message "Failed to delete: $executable_path"
            fi
        fi
    done
    
    # Clean up any XMRig configuration files
    ...
    for pattern in "${config_patterns[@]}"; do
        for dir in "${search_dirs[@]}"; do
            if [ -d "$dir" ]; then
                while IFS= read -r -d '' file; do
                    if [ -f "$file" ]; then
                        log_message "Deleting XMRig config file: $file"
                        rm -f "$file" 2>/dev/null
                        if [ $? -eq 0 ]; then
                            ((files_deleted++))
                            log_message "Successfully deleted: $file"
    ...
    log_message "XMRig cleanup completed. Total files deleted: $files_deleted"
}
...
# Clean up XMRig files at the end of the script
cleanup_xmrig_files
