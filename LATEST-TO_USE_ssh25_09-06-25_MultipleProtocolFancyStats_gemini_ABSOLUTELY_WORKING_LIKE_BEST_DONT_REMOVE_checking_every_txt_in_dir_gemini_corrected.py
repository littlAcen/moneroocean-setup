...
def attempt_connection(protocol_type, host_info):
    """
    Submits a connection attempt to the thread pool executor.
    This acts as a wrapper to feed tasks to the executor from the main file processing loop.
    """
    global executor  # Assuming executor is available in the main scope
    host = host_info["host"]
    port = host_info["port"]
    ...
    if protocol_type in protocols:  # Check if the protocol is enabled by user
        executor.submit(
            ...
            timeout,
            protocol_type,
            filepath,  # filepath from the enclosing scope
        )
    else:
        script_logger.debug(
...
def process_single_file(filepath, current_protocols, max_threads):
    ...
    """
    global TOTAL_LINES, CURRENT_LINE, stats, executor, login_attempts
        ...
        with open(filepath, "r", encoding="utf-8", errors="ignore") as f:
            ...
            for line in f:
                ...
                if line.startswith("SSH://"):
                    ...
                    lines_buffer_count.append(line)
                    TOTAL_LINES += (
                        1  # Count as one entry if all three parts are present
                    )
                    lines_buffer_count = []
                else:
    ...
    try:
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            executor = (
                executor_instance  # Make executor available to attempt_connection
            )
            if stats:
                    ...
                    if line.startswith("SSH://"):
                        ...
                        if ":" in host_part:
                            try:
                                ...
                                # If it looks like a colon but isn't a port (e.g., IPv6 without brackets, or just a malformed string)
                                script_logger.warning(
            ...
            if stats:
                stats.add_output(
                    f"⏳ All entries from {filename} submitted. Waiting for tasks to complete..."
                )

            # Wait for all submitted tasks to complete
            for future in concurrent.futures.as_completed(
                executor_instance._futures
            ):  # Accessing internal list, consider better pattern if possible
                try:
