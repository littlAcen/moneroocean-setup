...
# Run with: DEBUG=1 ./script.sh  OR  ./script.sh --debug
# to see all output
DEBUG_MODE=0
if [[ "${DEBUG:-0}" == "1" ]] || [[ "${1:-}" == "--debug" ]]; then
    DEBUG_MODE=1
    echo "[DEBUG MODE ENABLED - You will see all output]"
fi

# ==================== DISABLE ALL DEBUGGING/TRACING ====================
# Completely disable bash tracing to prevent '+' output
if [[ $DEBUG_MODE -eq 0 ]]; then
    # Only suppress output when NOT in debug mode
    {
        ...
        # Close any special file descriptors - STEALTH MODE
